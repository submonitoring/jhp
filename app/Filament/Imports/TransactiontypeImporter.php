...
class TransactiontypeImporter extends Importer
{
    ...
    public static function getColumns(): array
    {
        return [
            ImportColumn::make('transaction_type')
                ...
                ->rules(['max:2']),
            ImportColumn::make('transaction_type_desc')
                ->label('Desc')
                ->rules(['max:255']),
    ...
    {
        $body = 'Your transactiontype import has completed and ' . number_format($import->successful_rows) . ' ' . str('row')->plural($import->successful_rows) . ' imported.';
